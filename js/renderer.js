...
class ChessRenderer {
    constructor() {
        ...
        /* 🎥 開場視角(重置視角會放回這裡)。只寫這一份,兩邊都讀它。
           ...
           2026-09-13 曾照使用者「棋盤朝上,順時鐘 8 度,接近 2D 視角」改成 (0,-43.3,90)= 64.3°(SW v28 / v23),
           使用者實機看過後說「棋盤角度恢復上一版的角度 56.3 度,新版角度不 OK」⇒ **改回 (0,-60,90) = atan(90/60) = 56.3°**,
           和對局場同角。⚠ 這一站的角度使用者已經親自否決過一次「更陡」,再動之前先問。 */
