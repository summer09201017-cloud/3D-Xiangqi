...
class ChessRenderer {
    constructor() {
        ...
        /* 🎥 開場視角(重置視角會放回這裡)。只寫這一份,兩邊都讀它。
           ★ 只有**方向**有意義:fitCamera 會沿這個方向重算距離(照畫布長寬比裝滿),長度不重要。
           2026-09-13 使用者:「棋盤朝上,順時鐘 8 度,接近 2D 視角」⇒ 俯角從 atan(90/60)=56.3° 加 8°
           到 **64.3°** = atan(90/43.3)。角度越陡,平貼在棋子頂面的字被壓扁得越少、前後排的間距越平均;
           代價是近排的透視放大變少(棋盤看起來略窄)—— 本站是寬度卡住的直向型,影響小。
           (0909 之前這裡寫「和對局場同角度 56°」,從這一版起兩站不同角。) */
        this.INITIAL_CAM = { x: 0, y: -43.3, z: 90 };
    ...
    /* 📐 相機距離要**照畫布長寬比算**,不可以寫死(2026-09-09 補)。
       ...
       ★ 只改**距離**,不改俯角:方向沿用 INITIAL_CAM(0913 起 (0,-43.3,90) ⇒ 俯角 atan(90/43.3) ≈ 64°)。
         ⚠ 這個攤平公式的距離不隨俯角變:角度越陡,投影越接近攤平值 ⇒ 1.06 的餘裕只會更夠,不會裁到角。 */
    fitCamera() {
